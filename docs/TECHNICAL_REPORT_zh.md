...
**主线配置：** SmolLM2-360M 架构 · EN/ZH **7:3** 混合语料 ~7B tokens（FineWeb-Edu + FineWeb-2 `cmn_Hani`）· seed 42 已完成  
...
本实验在**同一模型结构、同一数据预算、同一随机初始化协议**下，对比五条路线（TE 硬件低精度，无软件 fake-quant）：
...
1. **R4 几乎贴齐 R1**（MXFP8 仅推理，质量损失可忽略）。  
2. **R2 最差**（BF16 权 + NVFP4 推）；**R3 明显好于 R2**（NVFP4 训练有帮助）。  
3. R5 相对 R1 约 +2% PPL；训练吞吐上 MXFP8 4 卡 best 与 BF16 基本持平，NVFP4 约 **0.90–0.92×**。  
4. 历史「纯英文 + 26.06」的绝对 PPL **不可**与本表对比；详见 [`EXPERIMENT_SUMMARY.md`](../EXPERIMENT_SUMMARY.md)。
